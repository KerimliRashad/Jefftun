// Один модуль на оба ядра.
//
// Их обязательно собирать вместе, а не двумя библиотеками. gomobile кладёт в
// каждую библиотеку полную копию среды выполнения Go, и при попытке слинковать
// две такие в один бинарник линковщик спотыкается на дубликатах: __cgo_topofstack,
// _crosscall2, _IncGoRef и прочих внутренностях Go. Здесь среда выполнения одна.
//
// Версии закреплены намеренно. Оба ядра тянут за собой десятки общих
// зависимостей, и Go выбирает старшую из требуемых версий — «последние» ядра
// однажды разъедутся так, что вместе перестанут собираться. Обновлять их
// нужно осознанно и с проверкой.
module zyng/core
...
require (
	...
	github.com/xtls/libxray v1.260909.0
)
...
require (
	...
	github.com/xtls/xray-core v1.260327.1-0.20260908222543-52a412d9e2f5 // indirect
