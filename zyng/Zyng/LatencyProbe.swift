...
func probeOnce(_ raw: String) async -> Int? {
    ...
    // Соединяемся ПО ИМЕНИ, а не по разрешённому нами адресу.
    //
    // Какое-то время здесь стояло обратное: мы разрешали имя своим защищённым
    // резолвером и подставляли числовой адрес. Сделано это было против подмены
    // DNS оператором — версия, которая потом не подтвердилась: Cloudflare
    // вернул ровно тот же адрес, что и оператор.
    //
    // А вреда та подстановка принесла много. Сотовые сети давно раздают только
    // IPv6, а до IPv4-серверов пускают через NAT64: система сама подставляет
    // нужный адрес, но делает это ТОЛЬКО когда ей дают имя. Готовому числовому
    // IPv4 она ничем помочь не может — маршрута к нему в такой сети нет вовсе.
    //
    // Отсюда и картина: на Wi-Fi все серверы отвечают, на 5G все до одного
    // «не отвечают», и поверх этого всплывает наша же плашка «подписка
    // устарела». Подписка была ни при чём.
