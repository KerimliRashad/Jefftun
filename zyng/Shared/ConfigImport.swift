...
enum ConfigImport {
    ...
    static func keys(fromJSON text: String) -> [String] {
        ...
        var result: [String] = []
        // Имена профилей, которые уже попали в список.
    ...
    private static func keys(fromConfig config: [String: Any],
        ...
        for outbound in outbounds {
            ...
            // Один сервер — одна запись.
            //
            // Панель описывает каждую страну несколькими выходами сразу:
            // vless по tcp, он же по xhttp, плюс Hysteria 2. Все они ведут на
            // ОДИН И ТОТ ЖЕ сервер и называются одинаково. Мы честно
            // раскладывали их по отдельным строкам — и из шести стран выходило
            // тридцать три записи, где «Netherlands — быстрый» повторялся
            // трижды подряд. Выбрать из такого списка невозможно.
            //
            // Берём первый выход профиля: панель ставит первым тот, которым и
            // предлагает пользоваться. Так же показывает Happ — по строке на
            // страну, а запасные пути остаются его внутренним делом.
            guard usedSignatures.insert(name).inserted else { continue }
