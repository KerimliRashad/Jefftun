...
enum SingBoxConfig {
    ...
    static func makeConfig(from key: String,
        ...
        var log: [String: Any] = [
            ...
            // каждое соединение вместе с адресом назначения. Довод верный, но
            // цена оказалась слишком высокой. На warn ядро молчит, пока
            // считает, что всё хорошо, — а «туннель поднят, трафик не идёт»
            // выглядит для него именно так. В журнале была строка «Ядро ничего
            // не записало», и разбирать было нечего: ни адреса, ни ошибки, ни
            // даже подтверждения, что соединение вообще пробовали открыть.
            //
